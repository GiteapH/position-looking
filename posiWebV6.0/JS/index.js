...
(function () {
	...
	myChart.on('click', function (params) {
		...
		timeFn = setTimeout(function () {
			...
			reload(name.replaceAll(getREGX(), "").substring(0,2), select.value, preCode.length >= 2, select.selectValue.label == '不限' ? null : select.selectValue.label, select.value)
			tempAddress = name.replaceAll(getREGX(), "").substring(0,2)
			return
		}, 250);
	});
	...
	myChart.on('dblclick', function (params) {
		...
		if (preCode.length != 1) {
			...
			reload(codeAdd[1].replaceAll(getREGX(), "").substring(0,2), select.value, preCode.length > 2, select.selectValue.label == '不限' ? null : select.selectValue.label, select.value)
			tempAddress = codeAdd[1].replaceAll(getREGX(), "").substring(0,2)
		} else {
			return
		}
	});
						...
						for (var i = 0; i < data.features.length; i++) {
							...
							if (tName != "") {
								proc.push(new Promise((resolve) => {
									getAddress(tName.replaceAll(getREGX(), ""))
										.then((ret) => {
...
function getREGX() {
	return /[省|市|自治区|壮族自治区|回族自治区|维吾尔族自治区]/g
}
