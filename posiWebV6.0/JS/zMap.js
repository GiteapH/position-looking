...
(function () {
	...
	myChart.on('click', function (params) {
		...
		timeFn = setTimeout(function () {
			...
			reloadPosisalary(name.replaceAll(/[省|市|自治区|壮族自治区|回族自治区|维吾尔族自治区]/g, "").substring(0,2), select.start, select.end)
	...
	myChart.on('dblclick', function (params) {
		...
		if (preCode.length != 1) {
			...
			reloadPosisalary(codeAdd[1].replaceAll(/[省|市|自治区|壮族自治区|回族自治区|维吾尔族自治区]/g, "").substring(0,2), select.start, select.end)
					...
					for (var i = 0; i < data.features.length; i++) {
						...
						if (tName != "") {
							proc.push(new Promise((resolve) => {
								ajaxModule("SELECT AVG(SALEAVG) AS COUNT FROM POSIINFO.ADDRESSDINFO WHERE ADDRESS LIKE '%" + tName.replaceAll(/[省|市|自治区|壮族自治区|回族自治区|维吾尔族自治区]/g, "") + "%' GROUP BY ADDRESS")
									.then((ret) => {
					...
					Promise.all(proc).then((res) => {
						...
						var option = {
							...
							visualMap: {
								...
								max: max,
								inRange: {
									color: ['rgba(175,238,238,0.9)', 'rgba(70 ,130 ,180,0.9)'
									]
								},
								splitNumber: 0,
								bottom: 100,
